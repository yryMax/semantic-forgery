...
parser = argparse.ArgumentParser(
    description="reprompt", parents=[gs_parser, tr_parser]
)
...
parser.add_argument(
    ...
    type=int,
    default=0,
    choices=list(range(len(PROMPTS_SD_LIST))),
)
parser.add_argument("--target_prompt", type=str, default=None)
parser.add_argument(
    "--attacker_prompt_index",
    type=int,
    default=0,
    choices=list(range(len(PROMPTS_I2P_LIST))),
)
...
parser.add_argument(
    "--wm_type", type=str, default="GS", choices=[wm.name for wm in WmProviders]
)
...
target_prompt = (
    PROMPTS_SD_LIST[args.target_prompt_index]
    if args.target_prompt is None
    else args.target_prompt
)
# prompts are taken from a predefined list of I2P (https://huggingface.co/datasets/AIML-TUDA/i2p)
attacker_prompt = (
    PROMPTS_I2P_LIST[args.attacker_prompt_index]
    if args.attacker_prompt is None
    else args.attacker_prompt
)
...
with torch.no_grad():
    ...
    res_1 = pipe_provider_target.generate(
    ...
    # log
    print(
        f"(Benign image) detection_success: {detection_successful}, bit accuracy: {results['bit_accuracy']:.5f}, p_value: {results['p_value']}, psnr: {results['psnr']:.5f}"
    )
    ...
    res_2 = pipe_provider_attacker.invert_images(
        images=res_1["images_torch"],
    ...
    res_3 = pipe_provider_attacker.generate(
    ...
    # log
    print(
        f"(Harmful image) detection_success: {detection_successful}, bit accuracy: {results['bit_accuracy']:.5f}, p_value: {results['p_value']}, psnr: {results['psnr']:.5f}"
    )
